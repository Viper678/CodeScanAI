...
- Data-retention deletes: `DELETE /uploads/{id}` (new — wipes raw + extracted disk artifacts and cascades through files / scans / scan_files / scan_findings via a schema-fixing migration) plus `DELETE /scans/{id}`; both wired into the UI as inline two-step confirmation buttons on `/uploads`, `/scans`, the upload tree-preview header, and the scan progress header
